...
// Page Sections - EXACTLY MATCHING YOUR FILE NAMES
import Overview from './pages/Overview';
import BasicDetails from './pages/BasicDetails';
import AboutProject from './pages/AboutProject';
import Amenities from './pages/Amenities';
import FloorPlan from './pages/floorplan';       // Updated to match floorplan.jsx
import NearbyLocations from './pages/NearbyLocations'; 
import Media from './pages/media';               // Updated to match media.jsx
import Reviews from './pages/review';            // Updated to match review.jsx
import ReraAndLegal from './pages/reraandlegal'; // Updated to match reraandlegal.jsx
import FAQ from './pages/faq';                   // Updated to match faq.jsx
...
const BuilderLayout = () => {
  ...
  const tabList = [
    ...
    { id: "nearby-locations", label: "Nearby Locations" },
    { id: "media", label: "Media" },
    { id: "reviews", label: "Reviews" },
    { id: "rera-legal", label: "RERA and legal" },
    { id: "faq", label: "FAQ" },
  ];

  // 2. TAB CONTENT (Strict Order matching above)
  const componentsList = [
    <Overview key="0" />,
    <BasicDetails key="1" />,
    <AboutProject key="2" />,
  ...
  return (
    <div className="max-w-[1600px] mx-auto w-full space-y-6">
      ...
      <div className="flex justify-between items-center">
        <h1 className="text-[22px] font-black text-slate-800 tracking-tight">Project Details</h1>
        <button className="bg-[#238EEB] hover:bg-[#1b7acb] transition-colors text-white text-[12px] font-bold py-2.5 px-5 rounded-lg flex items-center gap-2 shadow-sm">
          <Eye className="w-4 h-4" /> View Report
        </button>
      </div>

      {/* Main Grid: Left Card + Right Tabs */}
      <div className="grid grid-cols-12 gap-6 items-start">
        
        {/* Left Sidebar (Project Card) */}
        <div className="col-span-12 xl:col-span-3 sticky top-6">
          <ProjectProfileCard />
        </div>
